...
function createStore() {
	...
	var ret = function (config, initialState) {
		...
		var enhancers = config && config.enhancers ? [].concat((0, _toConsumableArray3.default)(config.enhancers)) : [];
		...
		if (sagas.length) {
			sagas.forEach(function (saga) {
				return runSaga(sagaMiddleware, saga);
			});
		}
		return store;
